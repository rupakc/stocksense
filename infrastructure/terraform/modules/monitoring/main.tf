...
resource "google_monitoring_notification_channel" "email" {
  ...
  display_name = "StockSense Admin Email"
  type         = "email"
  labels = {
...
resource "google_monitoring_uptime_check_config" "backend_health" {
  project      = var.project_id
  display_name = "StockSense Backend Health Check"
  timeout      = "10s"
  period       = "60s"
...
resource "google_monitoring_alert_policy" "uptime_failure" {
  project      = var.project_id
  display_name = "StockSense Backend Uptime Failure"
  combiner     = "OR"
...
resource "google_monitoring_alert_policy" "high_error_rate" {
  project      = var.project_id
  display_name = "StockSense High API Error Rate"
  combiner     = "OR"
...
resource "google_monitoring_alert_policy" "high_latency" {
  project      = var.project_id
  display_name = "StockSense High API Latency"
  combiner     = "OR"
...
resource "google_logging_metric" "api_request_count" {
  project = var.project_id
  name    = "stocksense_api_request_count"
...
resource "google_monitoring_dashboard" "main" {
  ...
  dashboard_json = jsonencode({
    displayName = "StockSense India — Production"
    mosaicLayout = {
      columns = 12
      tiles = [
        {
